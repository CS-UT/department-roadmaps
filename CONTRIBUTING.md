...
از علاقه شما به مشارکت در این پروژه ممنونیم! هدف ما این است که نقشه راه تمام گروه‌های دانشکده ریاضی، آمار و علوم کامپیوتر دانشگاه تهران را پوشش دهیم.

## افزودن نقشه راه گروه جدید

افزودن گروه جدید **نیاز به تغییر کد ندارد** — فقط کافی است یک فایل YAML بسازید.
...
name: فیزیک              # نام کامل گروه (فارسی)
...
| فیلد    | الزامی | توضیح                              |
| ------- | ------ | ---------------------------------- |
| `id`    | بله    | شناسه یکتای گروه (انگلیسی)        |
| `name`  | بله    | نام کامل گروه (فارسی)             |
...
| فیلد            | الزامی | توضیح                                                 |
| --------------- | ------ | ----------------------------------------------------- |
| `id`            | بله    | شناسه یکتای درس (با پیشوند گروه، مثلاً `ph-calc1`)   |
...
سرور توسعه را اجرا کنید و مطمئن شوید تب جدید ظاهر می‌شود و نقشه راه درست نمایش داده می‌شود.

#### ۵. ارسال Pull Request

تغییرات را Commit و Push کنید و یک Pull Request به مخزن اصلی ارسال کنید.

### نکات مهم

- شناسه دروس (`id`) باید **در کل پروژه یکتا** باشد. از پیشوند گروه استفاده کنید (مثلاً `ph-` برای فیزیک).
...
- اطلاعات دروس را از [سایت دانشکده](https://math.ut.ac.ir) یا سیستم گلستان استخراج کنید.
